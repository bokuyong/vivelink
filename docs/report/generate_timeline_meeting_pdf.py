...
def build_cover():
    ...
    summary = (
        ...
        "하드웨어 발주가 바로 진행되지 않아 필요한 장비는 사비로 먼저 구매해 개발을 이어갔고, "
...
def build_page_two():
    ...
    draw_box(
        ...
        "하드웨어 발주가 바로 진행되지 않아 기존 계획을 그대로 유지하기 어려웠다.\n\n"
        "이에 따라 이한솔이 보유 중이던 Radxa를 추가하여 3트랙 병행 구조로 수정했고, "
        "발주 대기 대신 사비로 장비를 먼저 구매하여 개발을 지속했고, 일정 관리도 주별 계획 대신 월별 계획 중심으로 다시 정리하였다.",
    ...
    y += 46
    y = draw_bullets(draw, [
        "하드웨어 발주가 바로 진행되지 않아 계획대로 장비 확보와 실험을 진행하기 어려웠음",
        "PA03는 실제 테스트 과정에서 성능 한계로 미러링이 안정적으로 유지되지 않았음",
        "프로젝트를 멈추지 않기 위해 사비로 하드웨어를 먼저 구매하고, 대체 보드인 Radxa를 추가하기로 결정했음",
...
def build_page_three():
    ...
    draw_paragraph(
        draw,
        "이한솔은 하드웨어 발주가 바로 진행되지 않는 상황에서 사비로 장비를 먼저 구매하여 개발이 중단되지 않도록 대응했고, "
...
def build_page_four():
    ...
    timeline = [
        (
            ...
            "하드웨어 발주가 바로 진행되지 않아 필요한 장비는 사비로 먼저 구매하는 방향으로 전환하였다."
        ),
        (
            "2026년 4월",
